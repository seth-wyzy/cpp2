...
class AIPlayer:
    ...
    def choose_bid(self, current_high: int) -> int:
        max_bid = self.bidding_meld + 10
        if current_high < 21: # Start at 21
            if 21 <= max_bid: return 21
            return 0 # Pass
...
class Game:
    def __init__(self):
        ...
        self.seat_assignments = [None] * 4 # Stores names of human players
        self.us_total = 0
        self.them_total = 0
        self.us_games = 0
        self.them_games = 0
    ...
    def is_human(self, seat_index: int):
        return self.seat_assignments[seat_index] is not None
    ...
    def reset_round(self):
        ...
        for i in range(4):
            self.hands[i] = sorted(self.deck[i*12 : (i+1)*12], key=lambda c: (c.suit, c.rank), reverse=True)
            self.players[i].start_round(self.hands[i])
        ...
        if not self.is_human(self.current_bidder):
            self.ai_bid_loop()

    def get_player_name(self, idx: int):
        if self.seat_assignments[idx]:
            return self.seat_assignments[idx]
        return f"AI ({['North', 'East', 'South', 'West'][idx]})"

    def ai_bid_loop(self):
        while self.phase == "bidding" and not self.is_human(self.current_bidder):
            ai_bid = self.players[self.current_bidder].choose_bid(self.bid)
    ...
    def handle_bid(self, player_idx: int, bid_amount: int):
        ...
        elif bid_amount > self.bid or (self.bid == 20 and bid_amount == 21):
            self.bid = bid_amount
        ...
        if active_count == 1:
            if self.bid_winner == -1: 
                ...
                self.bid = 20
                self.add_log(f"Everyone passed. {self.get_player_name(self.dealer)} stuck with 20.")
    ...
    def ai_play_one(self) -> bool:
        if self.phase == "trick_taking" and len(self.current_trick) < 4:
            curr_p = (self.trick_leader + len(self.current_trick)) % 4
            if not self.is_human(curr_p):
                card = self.players[curr_p].choose_move(self.current_trick, len(self.current_trick) == 0, self.trump)
                self.hands[curr_p].remove(card)
